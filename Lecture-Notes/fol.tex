...
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{I} = \mathtt{new Map([ [Q1, 4],\, [Q2, 8],\, [Q3, 1],\, [Q4, 3],\, 
                                 [Q5, 6],\, [Q6, 2],\, [Q7, 7],\, [Q8, 5]])}$.
\\[0.2cm]
If we define 
\\[0.2cm]
\hspace*{1.3cm}
$\mathcal{B} := \mathtt{new Map([ [Q1, 4],\, [Q2, 8],\, [Q3, 1]])}$.
\\[0.2cm]
...
\vbox{\offinterlineskip
  ...
  \caption{The partial assignment $\mathtt{new Map([ [Q1, 4],\, [Q2, 8],\, [Q3, 1]])}$.}
...
\begin{minted}[ frame         = lines, 
    ...
    type CSP                 = [string[], number[], string[]];          
    type Assignment          = Map<string, number>;
    type AnnotatedConstraint = [string, Set<string>];
    
    function collectVariables(formula: string): Set<string> {
        /* not yet interesting, wait for formal languages */
    }
...
\end{minted}
\vspace*{-0.3cm}
\caption{A backtracking \textsc{Csp} solver, part 1.}
...
\item A \texttt{CSP} is a list of length 3.
  \begin{enumerate}
  \item The first element is a list of the variables. Variables are strings.
  ...
  \item The third element is a list of formulas.
        ...
        These formulas are the constraints and they are represented as strings.
  \end{enumerate}
...
\item The function \texttt{collectVariables} takes a formula, which is represented as a string.  It returns the
...
\item The function \texttt{evaluateExpression} takes three arguments:
  \begin{enumerate}[(a)]
  \item \texttt{expr} is string.  This string is interpreted as TypeScript code that represents a Boolean
...
\item The function \texttt{isConsistent} takes five arguments:
  \begin{enumerate}[(a)]
  \item \texttt{variable} is a string. This represents the name of the variable we currently want to assign a value to.
...
\end{minted}
\vspace*{-0.3cm}
\caption{A backtracking \textsc{Csp} solver, part 2.}
...
\item The function \texttt{backtrackSearch} that is shown in Figure \ref{fig:CSP-Solver.ipynb-2} on page
  ...
  \item It checks if the size of the current \texttt{assignment} equals the total length of the
    ...
    is a solution of our \textsc{Csp} and hence is returned.
