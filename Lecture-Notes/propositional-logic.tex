...
\section{Sudoku and Logic-Solver}
...
is logically correct, solving the resulting set of over 10,000 clauses takes about 21 minutes on a modern
computer. To solve such problems efficiently, we can rely on industrial-strength SAT solvers. 

As described in the \texttt{README.md} file provided with its distribution,
\href{https://github.com/meteor/logic-solver}{\texttt{Logic-Solver}} is a boolean satisfiability solver
available as an \texttt{npm} package. It serves as a JavaScript wrapper around
\href{http://minisat.se/}{MiniSat}, an advanced C++ solver compiled to JavaScript via
Emscripten. \texttt{Logic-Solver} abstract away the manual conversion to Conjunctive Normal Form (CNF) by
allowing us to build formulas using high-level methods like \texttt{Logic.or}, \texttt{Logic.and}, and
\texttt{Logic.exactlyOne}. 

This section extends the concepts previously discussed in \texttt{propositional-logic.tex} by demonstrating how
to encode and solve the same Sudoku puzzle using this optimized library. We will explore the implementation
found in the notebook \texttt{11-Logic-Solver-Sudoku.ipynb}. 

\subsection{Implementation of the Sudoku Solver}
We begin by defining the basic types and a utility function to generate sequences of numbers. This is shown in
Figure \ref{fig:logicsolver-range}. 
...
\end{minted}
\vspace*{-0.3cm}
  \caption{Type definitions and the \texttt{range} function.}
  \label{fig:logicsolver-range}
\end{figure} %$

The TypeScript types \texttt{Variable}, \texttt{NegatedVariable}, \texttt{Literal}, and \texttt{Clause}
establish how boolean logic primitives are formatted as strings. The \texttt{range} function acts as a helper
to easily generate arrays of numbers, such as $[1, \dots, 9]$, which we need for iterating over the Sudoku
grid. 
...
\vspace*{-0.3cm}
  ...
  \label{fig:logicsolver-createPuzzle}
\end{figure}

The \texttt{createPuzzle} function (Figure \ref{fig:logicsolver-createPuzzle}) returns the puzzle as a
two-dimensional array. Given numbers are integers, and unknown fields are marked with a \texttt{"*"}.  

To encode the board in propositional logic, we generate variable names using the \texttt{varName} function, and
we can negate them using the \texttt{complement} function, both shown in Figure \ref{fig:logicsolver-varName}. 
...
\end{minted}
\vspace*{-0.3cm}
  \caption{The functions \texttt{varName} and \texttt{complement}.}
  \label{fig:logicsolver-varName}
\end{figure} %$

The \texttt{varName} function creates strings like \texttt{"Q<1,2,3>"} to represent the boolean variable
stating that row 1, column 2 holds the digit 3. The \texttt{complement} function returns the logical negation
of a literal by either adding or removing a leading minus sign (\texttt{-}), satisfying the format expected by
\texttt{Logic-Solver}. 

We now define the logical constraints, starting with the requirement that a specific digit must appear exactly
once within a given set of board positions. 
...
\begin{minted}[ frame         = lines, 
    ...
    function exactlyOneForDigit(L: Position[], digit: number): Logic.Formula {
        const variables = L.map(([row, col]) => varName(row, col, digit));
        return Logic.exactlyOne(variables) as Logic.Formula;
    }
\end{minted}
\vspace*{-0.3cm}
  \caption{The function \texttt{exactlyOneForDigit}.}
  \label{fig:logicsolver-exactlyOneForDigit}
\end{figure}

In Figure \ref{fig:logicsolver-exactlyOneForDigit}, we define \texttt{Position} as a tuple of row and
column. The \texttt{exactlyOneForDigit} function leverages \texttt{Logic.exactlyOne}, a powerful cardinality
constraint built directly into \texttt{Logic-Solver}. It takes a list of positions \texttt{L} and ensures that
the specified \texttt{digit} is placed in exactly one of those positions. 
To apply this to all digits, we use the function \texttt{exactlyOneForEveryDigit}.
...
\begin{minted}[ frame         = lines, 
    ...
    function exactlyOneDigit(row: number, col: number): Logic.Formula {
        const variables = range(1, 9).map(d => varName(row, col, d));
        return Logic.exactlyOne(variables) as Logic.Formula;
    }
\end{minted}
\vspace*{-0.3cm}
  \caption{The function \texttt{exactlyOneForEveryDigit}.}
  \label{fig:logicsolver-exactlyOneForEveryDigit}
\end{figure}

The \texttt{exactlyOneForEveryDigit} function (Figure \ref{fig:logicsolver-exactlyOneForEveryDigit}) maps over
the numbers 1 through 9, returning an array of formulas. This effectively states that within the given set of
coordinates \texttt{L} (which will later represent a full row, column, or block), every digit from 1 to 9 must
occur exactly once. 
...
\begin{minted}[ frame         = lines, 
    ...
    function exactlyOneDigit(row: number, col: number): Logic.Formula {
        return Logic.exactlyOne(
            range(1, 9).map(d => varName(row, col, d))
        ) as Logic.Formula;
    }
\end{minted}
\vspace*{-0.3cm}
  \caption{The function \texttt{exactlyOneDigit}.}
  \label{fig:logicsolver-exactlyOneDigit}
\end{figure}

Furthermore, we must ensure that every single cell contains exactly one digit.  The \texttt{exactlyOneDigit}
function in Figure \ref{fig:logicsolver-exactlyOneDigit} enforces that for a fixed \texttt{row} and
...
\begin{minted}[ frame         = lines, 
    ...
    function constraintsFromPuzzle(): Logic.Formula {
        const Puzzle = createPuzzle();
        const Facts  = [];
        for (let r = 1; r <= 9; ++r) { 
            for (let c = 1; c <= 9; ++c) {
                if (Puzzle[r-1][c-1] != '*') {
                    Facts.push(varName(r, c, Puzzle[r-1][c-1] as number));
                }
            }
        }
        return Logic.and(Facts) as Logic.Formula;
    }
\end{minted}
\vspace*{-0.3cm}
  \caption{The function \texttt{constraintsFromPuzzle}.}
  \label{fig:logicsolver-constraintsFromPuzzle}
\end{figure}

We must also convert the initial numbers from the puzzle grid into logical facts.
The function \texttt{constraintsFromPuzzle}  (Figure \ref{fig:logicsolver-constraintsFromPuzzle}) iterates over
the $9 \times 9$ array. For every non-asterisk entry, it generates the corresponding variable string and adds
it to the \texttt{Facts} array. It then combines all these facts using \texttt{Logic.and}, creating a single
formula that anchors the starting state of the board. 
...
\begin{minted}[ frame         = lines, 
    ...
    function getBlockCells(r: number, c: number): Position[] {
        let Result = [];
        for (let row = 1; row <= 3; ++row) {
            for (let col = 1; col <= 3; ++col) {
                Result.push([r * 3 + row, c * 3 + col]);
            }
        }
        return Result;
    }
...
\vspace*{-0.3cm}
  ...
  \label{fig:logicsolver-getBlockCells}
\end{figure}

To validate the $3 \times 3$ macro-blocks, we need a helper function to calculate coordinate translations.
The \texttt{getBlockCells} function in Figure \ref{fig:logicsolver-getBlockCells} takes the 0-based index of a
macro-block (\texttt{r} and \texttt{c}, both spanning 0 to 2) and returns the nine 1-based coordinates
\texttt{[row, col]} on the board that belong to that block. 
...
\begin{minted}[ frame         = lines, 
    ...
    function allConstraints(solver: Logic.Solver): Logic.Solution {
        // 1. Start with constraints from the puzzle
        solver.require(constraintsFromPuzzle());
        // 2. There is exactly one digit in every field
        for (let row = 1; row <= 9; ++row) {
            for (let col = 1; col <= 9; ++col) {
                solver.require(exactlyOneDigit(row, col));
            }
        }
        // 3. All entries in a row are unique
        for (let row = 1; row <= 9; ++row) {
            solver.require(
                exactlyOneForEveryDigit(range(1, 9).map(col => [row, col]))
            );
        }
        // 4. All entries in a column are unique
        for (let col = 1; col <= 9; ++col) {
            solver.require(
                exactlyOneForEveryDigit(range(1, 9).map(row => [row, col]))
            );
        }
        ...
        for (let row = 0; row <= 2; ++row) {
            for (let col = 0; col <= 2; ++col) {
                solver.require(
                    exactlyOneForEveryDigit(getBlockCells(row, col))
                );
            }
        }
        return solver.solve();
...
\vspace*{-0.3cm}
  \caption{The function \texttt{allConstraints}.}
  \label{fig:logicsolver-allConstraints}
\end{figure}

With all helpers in place, we construct the master function that binds everything together.
The \texttt{allConstraints} function (Figure \ref{fig:logicsolver-allConstraints}) takes a
\texttt{Logic.Solver} instance and sequentially feeds it all our rules via \texttt{solver.require(...)}. It
ensures: 
\begin{enumerate}
...
\end{enumerate}
Finally, it calls \texttt{solver.solve()}, executing MiniSat internally, and returns the \texttt{Logic.Solution} object. 

Executing this entire program yields a huge performance improvement.  While the Davis-Putnam
alternating spent 21 minutes laboring through the SAT derivation, the Jupyter notebook that uses \texttt{Logic-Solver}
accomplishes the same goal in roughly 60 milliseconds.  
The Jupyter notebook showing the resulting program is available at:
\\[0.2cm]
\hspace*{1.3cm}
\href{https://github.com/karlstroetmann/Logic/blob/master/TypeScript/Chapter-4/11-Logic-Solver-Sudoku.ipynb}{\texttt{Chapter-4/11-Logic-Solver-Sudoku.ipynb}}
\pagebreak
