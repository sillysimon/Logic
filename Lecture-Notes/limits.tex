...
\epigraph{\blue{The only reliable way to test if a program will run forever is to run it \red{forever}.}}{
— \calligfont Anonymous}
...
have trouble \href{http://www.youtube.com/watch?v=RUMX_b_m3Js}{to walk on water}.
Only very experienced \href{https://youtu.be/3gLwE44yieY}{bishops} can do it.
Similarly, computer science has its limits.  We will discuss 
these limits next.
First, we show that we cannot decide whether a computer program will eventually
...
another computer program does terminate.
...
terminate.
Consider the program shown in Figure \ref{fig:legendre.stlx} on page
\pageref{fig:legendre.stlx}.  This program contains a \texttt{while}-loop in line 23.  
If there is a natural number $k \geq n$ such that the expression,
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{legendre}(k)$
\\[0.2cm]
in line 24 evaluates to \texttt{false}, then the program prints a message and terminates.
However, if 
$\mathtt{legendre}(k)$ is true for all $k \geq n$, then the \texttt{while}-loop does not terminate.
...
to \texttt{false} for this value of $n$.
The function \texttt{legendre} is defined in line 10.  
...
\\[0.2cm]
holds.
Although there are a number of arguments in support of Legendre's conjecture,  to this day
nobody has been able to prove it.
The answer to the question, whether the invocation of the function \texttt{findCounterExample} will
...
had some procedure that could check whether the function call $\texttt{findCounterExample}(1)$ does terminate,
then this procedure would be able to decide whether Legendre's theorem is true.
Therefore, it should come as no surprise that such a procedure does not exist.
...
function divisors(k: number): number[] {
    return range(k).filter(t => k % t == 0);
}

function isPrime(k: number): boolean {
    const divisorList = divisors(k);
    return k != 1 && divisorList.length == 2;
}
...
function findCounterExample(n: number): void {
    while (true) {
        if (legendre(n)) {
            ...
            console.log(`No prime between ${n}**2 and ${n + 1}**2!`);
            return;
        }
    }
}
...
Let us proceed to prove formally that the halting problem is not 
solvable.  To this end, we need the
...
\hspace*{1.3cm} \texttt{\textasciigrave}\texttt{function $f$(x) \{} \\
\hspace*{1.8cm} \textsl{body}        \\
\hspace*{1.45cm} \texttt{\}\textasciigrave}         \\[0.2cm]
and, furthermore, the string $t$ can be parsed as a \textsl{TypeScript} function, that is the evaluation of
the expression
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{eval}(t)$
...
The set of all test functions is denoted as $T\!F$.
If $t \in T\!F$ and $t$ has the name $f$, then
this is written as 
\\[0.2cm]
\hspace*{1.3cm}
$\mathtt{name}(t) = f$.
...
\item We define the string $s_3$ as
      ...
      Then $s_3$ is not a test function.
      The reason is that in \textsl{TypeScript} the body of a function needs to be enclosed in braces.
...
If $n$ is the name of a \textsl{TypeScript} function that takes $k$ arguments $a_1$, $\cdots$, $a_k$,
then we write 
\\[0.2cm]
\hspace*{1.3cm}
...
\end{enumerate}
If there was a \textsl{TypeScript} function \texttt{stops} that did satisfy the specification given above,
then the halting problem for \textsl{TypeScript} would be \blue{decidable}.
\index{decidable}
...
there can be no function \texttt{stops} satisfying the specification given above.
This calls for an
...
exist and we will then show that this assumption leads to a contradiction.
...
In order to proceed, let us assume that a \textsl{TypeScript} function \texttt{stops}
...
In order to do this, we have to check what happens when
      the expression
      \\[0.2cm]
      \hspace*{1.3cm}
      ...
      \item Line 3 now tests whether \texttt{result} is $1$.
Of course,
            this test fails.
Therefore, the block of the \texttt{if}-statement is not executed.
...
\end{enumerate}
      ...
      \textsl{turing}.
However, this is the opposite of what the function \texttt{stops} has claimed.
...
Therefore, the block of the \texttt{if}-statement \underline{is} executed.
      \item However, this block contains an infinite loop.
Therefore, the
...
\cite{turing:1937}.
Of course, Turing did not solve the problem for \textsl{TypeScript} but rather
for the so called 
...
However, if you check the proof given for \textsl{TypeScript} you will easily
...
halting problem for this weak programming language.
But for any programming language that supports
...
\exerciseEng
A set $X$ is \blue{countable}\index{countable} iff there is a function 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm]
holds.
Next, and this is the crucial step, define a set \texttt{Cantor} as follows:
\\[0.2cm]
...
is countable.
The set $\mathbb{Q}$ can be defined as follows: 
\\[0.2cm]
\hspace*{1.3cm}
...
Unfortunately, the halting problem is not the only undecidable problem in computer science.
Another
important problem that is undecidable is the question whether two given functions always compute the
same result.
To state this more formally, we need the following definition.
\begin{Definition}[$\simeq$] 
Assume $n_1$ and $n_2$ are the names of two \textsl{TypeScript} functions that take arguments
  $a_1$, $\cdots$, $a_k$.
Let us define \\[0.2cm]
\hspace*{1.3cm} 
...
\noindent
We are now ready to state the \blue{equivalence problem}.
A \textsl{TypeScript} function \texttt{equal} solves the
\emph{equivalence problem} \index{equivalence problem} if it is defined as
\\[0.2cm]
\hspace*{1.3cm} \texttt{function equal(p1, p2, a) \{}               \\
...
Therefore, assume that there is a function \texttt{equal}
such that \texttt{equal} solves the equivalence problem.
Assuming \texttt{equal} exists, we will
...
return 1 if and only the evaluation of $n(a)$ terminates.
As we have already proven that the
...
that we should not attempt to do so.
After all, we only have proven that there is no procedure that
can \underline{alwa}y\underline{s} check whether a given program will terminate.
There might well exist a
procedure for termination checking that works most of the time.
Indeed, there are a number of
systems that try to check whether a program will terminate for every input.
For example, for
...
\cite{mueller:1996} describes a successful approach.
The recent years have seen a lot of progress in
this area.  The article 
``\href{http://dl.acm.org/citation.cfm?id=1941509}{{Proving Program Termination}}''
\cite{cook:2011} reviews these developments.
However, as the recently developed systems rely on both
...
\item Show how the notion $\simeq$ is defined.
...
discusses the undecidability of the halting problem in section 4.2.
It also covers many related
...
Rajeev Motwani and Jeffrey D.~Ullman \cite{hopcroft:06}.
This book is the third edition of a
...
This increased formality is necessary to prove that, for example, it is undecidable
...
year.
If you want to dive deeper into the concept of undecidability, you should do so only after you have
